...
import { AreaLight } from '../lights/areaLight.js';
import { TubeLight } from '../lights/tubeLight.js';
import { RectLight } from '../lights/rectLight.js';
...
class Shader {
	...
	setParameter( key, value ) {
		...
		if ( key in this.uniforms ) {
			...
			if ( this.uniforms[ key ].value instanceof THREE.Color ) {
				...
				this.uniforms[ key ].value instanceof THREE.Color ||
				...
				this.uniforms[ key ].value instanceof THREE.CubeTexture ||
	...
	setArrayParameter( arrayKey, index, key, value ) {
		...
		if ( arrayKey in this.uniforms ) {
			...
			if ( key in this.uniforms[ arrayKey ].value[ index ] ) {
				...
				if (
					this.uniforms[ arrayKey ].value[ index ][ key ] instanceof THREE.Color ||
					...
					this.uniforms[ arrayKey ].value[ index ][ key ] instanceof THREE.CubeTexture ||
	...
	setLightParameter( index, light, camera ) {
		...
		if ( light instanceof THREE.DirectionalLight ) {
			...
			const start = new THREE.Vector3().copy( light.start );
			start.applyMatrix4( camera.matrixWorldInverse );
			const end = new THREE.Vector3().copy( light.end );
			...
			for ( let i = 0; i < light.positions.length; ++i ) {
				...
				const p = new THREE.Vector3().copy( light.positions[ i ] );
				p.applyMatrix4( matrix );
				positions.push( p );

			}

			const normal = new THREE.Vector3( 0, 0, 1 ).applyMatrix4( matrix );
			const tangent = new THREE.Vector3( 1, 0, 0 ).applyMatrix4( matrix );
